...
class SentimentAnalysisTrainer:
    def __init__(
        ...
        super(SentimentAnalysisTrainer, self).__init__()
        self.model = model
    ...
    def _train_epoch(self):
        ...
        for step, (inputs, attention_masks, labels) in enumerate(tqdm(self.train_loader)):
            ...
            pred_list.extend(outputs.logits.argmax(dim=1).tolist())

            if step % self.log_every == 0:
                train_mean_loss = total_loss / (step + 1)
                train_f1_score = f1_score(y_true=target_list, y_pred=pred_list, average='binary')
    ...
    def _validate(self, epoch):
        ...
        valid_f1_score = f1_score(y_true=target_list, y_pred=pred_list, average='binary')
